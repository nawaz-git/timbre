...
enum WordTimeline {
    ...
    static let minTurnSeconds: TimeInterval = 0.4
    ...
    struct Word: Equatable, Sendable {
        ...
        func shifted(by delta: TimeInterval) -> Word {
    ...
    struct AttributedWord: Equatable, Sendable {
        let word: Word
        var speaker: String
    }
    ...
    static func exclusiveTurns(
        from segments: [DiarizationResult.Segment],
    ...
    static func coalesce(_ turns: [Turn]) -> [Turn] {
    ...
    static func assign(words: [Word], turns: [Turn]) -> [AttributedWord] {
    ...
    static func joinWords(_ texts: [String]) -> String {
    ...
    static func attribute(
        words: [Word],
        diarization segments: [DiarizationResult.Segment],
        turnSpeakerMap: [String: String] = [:],
        maxPause: TimeInterval = defaultMaxPause,
    ) -> [TimestampedSegment] {
        let named = turnSpeakerMap.isEmpty ? segments : segments.map {
            DiarizationResult.Segment(start: $0.start, end: $0.end, speaker: turnSpeakerMap[$0.speaker] ?? $0.speaker)
        }
