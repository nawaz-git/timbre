 ...
 * native Settings are unreachable. The two recording-scope toggles therefore
 * live in Timbre's Settings UI and are propagated to the engine through a small
 ...
 *
 * The payload carries ONLY the two genuinely-new cross-process concepts —
 * `screenCaptureScope` and `recordMicrophone`. `recordScreenVideo` is
 * deliberately NOT included: the engine keeps its own AppSettings/UserDefaults
 * gate for video on/off, untouched by this bridge.
 *
